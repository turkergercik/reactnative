...
const{auth,navbar,setauth,messages,setmessages,currentconv,messageRef,userid,server,state,socket,setstat,rr,stat,authContext,check,allm,setss,cam,setimg,img,setsomemessages,somemessages,menuopens,onlines,inchat,typing,currentother,setlastmesssages,incall,setmpeop,mpeop:mp}=useAuthorization()
let prt= server
let na={
  id:state.userId
}
                 ...
                 <IconButton icon={"phone"} size={25} iconColor='white' rippleColor={"grey"} style={{ margin: 0 }} onPress={async() => {
                  ...
                  storage.set("calldetails",JSON.stringify({convid:id,otherid:otherid,notid:notid,entrytype:"call",callerName:other,type:"Audio"}))
                   pause.startcall(
                    ...
                    RNCallKeep.answerIncomingCall("45")
                    /* await notifee.displayNotification({
                      id:"2",
                      title:"arama",
                      body:"bok",
